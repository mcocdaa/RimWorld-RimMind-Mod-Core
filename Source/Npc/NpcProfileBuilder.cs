    ...
    public static class NpcProfileBuilder
    {
        public static NpcProfile BuildPawnNpc(Pawn pawn)
        {
            ...
            var profile = new NpcProfile
            {
                ...
                SystemPrompt = $"You are {pawn.Name?.ToStringShort ?? "a colonist"}, a colonist in a RimWorld colony.",
                Commands = new List<NpcCommand>
                {
                    new NpcCommand { Name = "express_emotion", Description = "Express an emotion or feeling", Parameters = "{\"emotion\":\"string\",\"intensity\":\"float\"}" },
                    new NpcCommand { Name = "change_relationship", Description = "Change relationship with another pawn", Parameters = "{\"target_name\":\"string\",\"delta\":\"float\"}", NeverRespondWithMessage = true },
                },
            };
        ...
        {
            if (pawn == null) return "Unknown pawn";
            var sb = new System.Text.StringBuilder();
            sb.AppendLine($"Name: {pawn.Name?.ToStringFull ?? pawn.LabelShort}");
            if (pawn.story?.Adulthood != null)
                sb.AppendLine($"Backstory: {pawn.story.Adulthood.title}");
            if (pawn.story?.traits?.allTraits != null)
            {
                ...
                if (traitNames.Count > 0)
                    sb.AppendLine($"Traits: {string.Join(", ", traitNames)}");
            }
            if (pawn.skills?.skills != null)
            {
                ...
                if (topSkills.Count > 0)
                    sb.AppendLine($"Skills: {string.Join(", ", topSkills)}");
            }
