...
{
    [Obsolete("L3: Use IContextKeyRegistry (ContextKeyRegistryImpl) via RimMindRuntime.ContextKeys or RimMindAPI.Context.ContextKeys. " +
        "This static class is retained only for RegisterCoreKeys() until the Migrate phase completes.")]
    public static class ContextKeyRegistry
    {
            ...
            bool isIndexable = false, float[]? keyEmbedding = null)
        {
            ...
            {
                var old = _keys[key];
                LogSink?.Warning($"[RimMind-Core] ContextKey '{key}' registered by '{old.OwnerMod}' " +
