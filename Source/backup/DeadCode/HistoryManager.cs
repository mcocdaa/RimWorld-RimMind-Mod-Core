...
using NpcHistoryEntry = RimMind.Application.Common.Models.Npc.NpcHistoryEntry;
...
namespace RimMind.Presentation.Context
{
    public class HistoryManager : IHistoryManager
    {
        ...
        {
            var entries = _histories.GetOrAdd(npcId, _ => new List<NpcHistoryEntry>());
            int tick = CurrentTick;
            lock (_listLock)
            {
                entries.Add(new NpcHistoryEntry("user", userMessage, tick, scenario));
                entries.Add(new NpcHistoryEntry("assistant", assistantMessage, tick, scenario));
            }
        }
        ...
        public void ReplaceLastAssistantTurn(string npcId, string content)
        {
            ...
            lock (_listLock)
            {
                for (int i = entries.Count - 1; i >= 0; i--)
                {
                    ...
                    {
                        entries[i] = new NpcHistoryEntry("assistant", content, entries[i].Tick, entries[i].Scenario);
                        break;
                    }
                }
            }
        ...
        public Dictionary<string, List<NpcHistoryEntry>> GetAllForSaveDict()
        {
            var result = new Dictionary<string, List<NpcHistoryEntry>>();
        ...
        internal void LoadFromSave(Dictionary<string, List<NpcHistoryEntry>> data)
        {
            _histories.Clear();
            if (data == null) return;
        ...
        public void ExposeData()
        {
            ...
            {
                LoadFromSave(dict ?? new Dictionary<string, List<NpcHistoryEntry>>());
            }
        }
    }
}
