...
5. **Provider 注册机制**：子模组通过 `ContextKeyRegistry.Register` 注入上下文 Provider，支持卸载与覆盖，实现解耦
6. **Agent 认知架构**：`PawnAgent` 统一认知主体（Perceive→Think→Act→Record），`AgentBus` 事件总线，`AgentGoalStack` 目标栈
7. **双路径 API**：`RimMindAPI.Chat()` (NpcChat 对话) + `RimMindAPI.RequestStructuredAsync()` (结构化/工具调用)
8. **请求审批悬浮窗**：`RequestOverlay` + `RequestEntry`，子模组可注册待审批请求供玩家选择
9. **SkipCheck 机制**：子模组可注册对话/浮菜单/动作/叙事者的跳过检查，实现互斥控制
10. **设置 UI**：多分页设置界面（API/队列/提示词/上下文），子模组可注册额外分页
11. **调试工具**：AI Debug Log 窗口、Dev DebugAction
...
│   │   ├── AgentGoalStack.cs 目标栈（3活跃/10总上限）
...
// ── 上下文构建（新路径：ContextEngine）──
RimMindAPI.GetContextEngine()  // 获取共享 ContextEngine 实例（public）
RimMindAPI.Chat(ContextRequest request, CancellationToken ct)  // NpcChat 对话路径
RimMindAPI.RequestStructuredAsync(AIRequest request, string jsonSchema, Action<AIResponse> onComplete, List<StructuredTool>? tools)  // 结构化路径

// ── 上下文构建（旧路径，已废弃）──
// BuildFullPawnPrompt / BuildFullPawnSections 已删除，请使用 ContextEngine.BuildSnapshot
...
2. L1_Baseline  → 版本化缓存 + Diff 注入，map_structure + pawn_base_info + fixed_relations + ideology + skills_summary + memory_narrator
3. Diff         → [环境突变] 消息，L1 key 值变化时记录，4轮后合并回 L1 block
4. L2_Environment → current_area + weather + time_of_day + nearby_pawns + season + colony_status
5. L3_State     → health + mood + current_job + combat_status + target_info + task_progress + memory_pawn + working_memory
6. L4_History   → HistoryManager 对话历史（按 Budget 调整轮数）
7. CurrentQuery → 当前用户输入
```

BudgetScheduler: Score = 0.4 * GetEffectivePriority() + 0.6 * ComputeRelevance(scenario, key)
- 有 IRelevanceProvider 时：ComputeRelevance = 0.6 * RelevanceTable + 0.4 * SemanticEmbedding
- 无 IRelevanceProvider 时：ComputeRelevance = RelevanceTable
...
|--------|---------|----------------|
| RimMind-Personality | `ContextKeyRegistry.Register` + `RegisterPawnContextProvider` | personality_profile + personality_state + personality_shaping + **AgentIdentity** |
| RimMind-Memory | `ContextKeyRegistry.Register` + `RegisterPawnContextProvider` + `RegisterStaticProvider` | memory_pawn (L3) + working_memory (L3) + memory_narrator (L1) |
...
- 人格通过 `ContextKeyRegistry.Register` + `RegisterPawnContextProvider` 注入上下文
- AgentIdentity 通过 `RegisterAgentIdentityProvider` 向 Core 注册，GoalGenerator 通过 `GetAgentIdentity` 读取
- 想法自动打包进入上下文（Thought 系统）
- 记忆通过 `ContextKeyRegistry.Register` (memory_pawn/working_memory/memory_narrator) 自动注入上下文
- 行动以记忆方式注入上下文
