...
namespace RimMind.Core.ArchTests.PhaseH2
{
    public class PhaseH2ArchTests
    {
        ...
        private static IEnumerable<string> GetProductionCsFiles(string root)
        {
            return Directory.GetFiles(root, "*.cs", SearchOption.AllDirectories)
                .Where(f => !f.Contains("\\obj\\") && !f.Contains("\\bin\\") && !f.Contains("\\Tests\\") && !f.Contains("\\ArchTests\\") && !f.Contains("\\Refs\\") && !f.Contains("\\_backup\\"));
        }
