...
namespace RimMind.Core.ArchTests.PhaseStructure
{
    public class R_S4_ColonyDirExistsTests
    {
        ...
        {
            var colonyDir = Path.Combine(SourceRoot, "Kernel", "Mechanisms", "Colony");
            Directory.Exists(colonyDir).Should().BeTrue(
                "Colony/ 目录为 P1/P2 机制预留");
        }

        [Fact, Trait("Phase", "Structure")]
        public void Kernel_AgentMode_Dir_Should_Exist()
        {
            var agentModeDir = Path.Combine(SourceRoot, "Kernel", "AgentMode");
            Directory.Exists(agentModeDir).Should().BeTrue(
                "AgentMode/ 目录为 I 阶段预留");
        }

        [Fact, Trait("Phase", "Structure")]
        public void Kernel_Pipeline_Unified_Dir_Should_Exist()
        {
            var unifiedDir = Path.Combine(SourceRoot, "Kernel", "Pipeline", "Unified");
